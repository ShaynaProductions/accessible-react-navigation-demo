...
const renderLinkFacade = (props: Partial<LinkFacadeInterface>) => {
  return render(
    <LinkFacade href="#" testId={TEST_ID} {...props}>
      {label}
    </LinkFacade>,
  );
};
...
describe("<LinkFacade />", () => {
  it("should be WCAG compliant", async () => {
    ...
    const { container } = renderLinkFacade(optProps);
  ...
  it("should not have an href when disabled", () => {
    ...
    const { getByTestId } = renderLinkFacade(optProps);
